...
- **009-stats-leaderboard**: Implemented post-session final leaderboard (GET `/sessions/:id/leaderboard`, with JSON and RFC 4180 CSV export via `?format=csv` and attachment header) and per-question breakdown statistics (GET `/sessions/:id/stats` with correct/wrong/no-answer counts and distribution) in `internal/session/`; status guard enforcing `completed`/`cancelled` statuses (HTTP 409 `SESSION_NOT_COMPLETED`); deterministic ranking tie-breaker (`total_score DESC, joined_at ASC, id ASC`); URL-safe filename slugification; verified session history filtering compatibility.
